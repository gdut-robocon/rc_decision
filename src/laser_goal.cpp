...
BT::NodeStatus rc_decision::laser_goal::onStart()
{
  ...
  if ( !getInput<Pose2D>("Goal_point", goal_point_))
  {
    throw BT::RuntimeError("missing required input [Goal_point]");
  }
  ROS_INFO("[ MoveBase: SEND REQUEST ]. goal: x=%f y=%f theta=%f\n",
         goal_point_.x, goal_point_.y, goal_point_.theta);


if(goal_point_.x!=0&&goal_point_.y!=0&&std::isfinite(goal_point_.x)&&std::isfinite(goal_point_.y))
{
 // Build the message from Pose2D
  move_base_msgs::MoveBaseGoal msg;
  msg.target_pose.header.frame_id = "map";
  msg.target_pose.header.stamp = ros::Time::now();
  msg.target_pose.pose.position.x = goal_point_.x;
  msg.target_pose.pose.position.y = goal_point_.y;
  ...
  ac_.sendGoal(msg);
  _aborted = false;
  while (!_aborted && !ac_.waitForResult(ros::Duration(0.02))) {
